...
where likes.photo_id = 3;


select username 
from users
join follows
on followee_id = users.user_id
where follower_id = 2;

select username from users where username = 'emily_brown';

select username
from users 
join likes 
on likes.photo_id = users.user_id 
where liked_at < '2021-09-13 12:00:00';
