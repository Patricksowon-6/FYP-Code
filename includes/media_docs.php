<?php
$stmt = $conn->prepare("
    SELECT uf.file_id, uf.path, uf.original_name, u.user_name, p.project_id, uf.file_approval
    FROM user_files uf
    JOIN users u ON uf.user_id = u.user_id
    JOIN projects p ON uf.project_id = p.project_id
    WHERE uf.category = 'documents' AND p.project_id = ?
    ORDER BY uf.uploaded_at DESC
");

$stmt->bind_param("i", $project_id);
$stmt->execute();
$result = $stmt->get_result();
?>

<?php if ($result->num_rows > 0): ?>

    <?php while ($doc = $result->fetch_assoc()):
        $url = rtrim(SUPABASE_URL, '/') . "/storage/v1/object/{$doc['path']}";
    ?>
        <?php if ($doc['file_approval'] === 'approved'): ?>
            <div class="video" style="margin-top: 100px;"data-asset-id="<?= $doc['file_id'] ?>">
                <center>
                    <img src="<?= IMG_PATH ?>document_icon.png" class="thumbnail-image documentPreviewBtn" alt="File" data-document="<?= htmlspecialchars($url) ?>">

                    <h1 class="file-name">
                        <?= htmlspecialchars($doc['original_name']); ?><br><br>
                    </h1>

                    <h3 class="person-name">
                        By <?= htmlspecialchars($doc['user_name']); ?><br><br>
                    </h3>

                    <div class="action-buttons">
                        <a href="<?= htmlspecialchars($url) ?>" download="<?= htmlspecialchars($doc['original_name']) ?>" target="_blank">
                            <i class="fa-solid fa-download"></i>
                        </a>

                        <button type="button" class="delete-btn"
                                onclick="openDeleteModal('<?= $doc['file_id'] ?>')">
                            <i class="fa-solid fa-trash"></i>
                        </button>

                        <a href="<?= PAGES_URL ?>comments_page.php?docs=<?= urlencode($url) ?>&project_id=<?= $project_id ?>&file_id=<?= $doc['file_id'] ?>">
                            <i class="fa-solid fa-comment"></i>
                        </a>

                        <a href="<?= PAGES_URL ?>versions.php?file_id=<?= $doc['file_id'] ?>">
                            <i class="fa-solid fa-list"></i>
                        </a>

                        <?php //include(INCLUDES_PATH . 'kebab.php'); ?>
                    </div>
                </center>
            </div>
        <?php endif; ?>
    <?php endwhile; ?>

<?php else: ?>

    <!-- EMPTY STATE (CONSISTENT ACROSS APP) -->
    <div class="empty-state">
        <div class="empty-box">
            <i class="fa-regular fa-file-lines empty-icon"></i>

            <h2>No documents yet</h2>
            <p>Add your first asset to get started with this project.</p>

            <a href="<?= PAGES_URL ?>file_upload.php" class="add-asset-btn">
                <i class="fa-solid fa-plus"></i> Add Asset
            </a>
        </div>
    </div>

<?php endif; ?>


<!-- DELETE CONFIRMATION MODAL -->
<div id="deleteModal" class="modal-overlay">
    <div class="modal-box">
        <h3>Are you sure?</h3>
        <p>This action cannot be undone. The asset will be permanently deleted.</p>
        <div class="modal-actions">
            <button class="cancel-btn" onclick="closeDeleteModal()">Cancel</button>
            <button class="confirm-btn" id="confirmDeleteBtn">Delete</button>
        </div>
    </div>
</div>


<!-- DOCUMENT MODAL -->
<div id="documentModal" class="modal">
    <div class="modal-content">

        <span class="close-doc">&times;</span>

        <h2>📄 Document Preview</h2>

        <iframe id="documentViewer" style="border:none;"></iframe>

    </div>
</div>

<script>

// Document Preview
document.addEventListener('DOMContentLoaded', () => {

    const documentModal = document.getElementById("documentModal");
    const documentViewer = document.getElementById("documentViewer");
    const closeDocument = documentModal.querySelector(".close-doc");

    document.querySelectorAll(".documentPreviewBtn").forEach(btn => {

        btn.addEventListener("click", () => {

            const src = btn.dataset.document;

            if (!src) return;

            documentViewer.src = src;

            documentModal.style.display = "flex";
        });
    });

    closeDocument.onclick = () => {

        documentModal.style.display = "none";

        documentViewer.src = "";
    };

    window.addEventListener("click", e => {

        if (e.target === documentModal) {

            documentModal.style.display = "none";

            documentViewer.src = "";
        }
    });
});

</script>